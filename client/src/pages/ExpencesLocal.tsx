...
const ExpencesLocal = () => {
  ...
  const addNewItem = (item: Item) => {
    ...
    if (item.category.length === 0) {
      ...
      const findItem = items.find(
        (inStateItem) =>
          inStateItem.name.toLowerCase() === item.name.toLowerCase() &&
          inStateItem.category.toUpperCase() !== DEFAULT_CATEGORY.toUpperCase()
      );
  ...
  const processRowUpdate = (newRow: ItemInState) => {
    ...
    if (!newRow.amount_bought) throw new Error("Missing item amount_bought");
    if (!newRow.price_per_unit) throw new Error("Missing item amount_per_unit");
  ...
  const getUniqueValues = (...args: Array<string>) => {
    ...
    const newArray = new Set(args.flat().map((value) => value.toLowerCase()));
  ...
  const mergeDuplicates = (items: ItemInState[]) => {
    ...
    items.forEach((item) => {
      ...
      const index = updatedItems.findIndex(
        (updatedItem) =>
          updatedItem.name.toUpperCase() === item.name.toUpperCase() &&
  ...
  const updateLocalStorage = (item: ItemInState) => {
    ...
    if (localStorageCategories) {
      const existingCategories = new Set(JSON.parse(localStorageCategories));
      ...
      localStorage.setItem("item-categories", JSON.stringify([item.category]));
    }
  };
  ...
  useEffect(() => {
    ...
    const categoriesInLocalStorage = localStorage.getItem("item-categories");
  ...
  return (
    <main className="flex flex-row gap-0 max-h-screen overflow-y-hidden">
      <section className="min-w-[350px] px-2 bg-blue-400 h-screen flex flex-col gap-1 py-2">
        ...
        <Button variant="outlined" onClick={() => mergeDuplicates(items)}>
          Merge
        </Button>
